...
export function inferScopeFromPath(normalizedPath: string): string | undefined {
  ...
  if (/\/api\/v1\/viajero-3-dias\//i.test(path)) return 'emissions:person';
  if (/\/api\/v1\/viajero-7-dias\//i.test(path)) return 'emissions:person';
  ...
  if (/\/api\/v1\/siniestros\/validar/i.test(path)) return 'endosos:write';
