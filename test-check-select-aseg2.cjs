...
async function run() {
  ...
  const query2 = `
    SELECT count(*) as cnt FROM peasegurados WHERE cnpoliza = '9-1-1000000964                '
  `;
  const res2 = await pool.request().query(query2);
  console.log('PEASEGURADOS ACTUAL COUNT:', res2.recordset[0].cnt);
