-- Portal condominio / Hogar: validar póliza vigente por cédula + plan + apartamento (certificado),
-- no solo cédula + ramo/plan. Aplicar en Sis2000 QA/PROD y actualizar sp_pre_emision_condominio_nexus
-- para pasar @ncertificado a speeValidatePersonGeneral.
--
-- Requiere: parámetro @ncertificado INT = NULL en speeValidatePersonGeneral.

ALTER PROCEDURE [dbo].[speeValidatePersonGeneral]
		@cramo INT,
		@cplan VARCHAR(10),
		@femision DATE = NULL,
		@fdesde DATE = NULL,
...
BEGIN
    ...
    END
		...
		IF @CPLAN NOT LIKE 'BANAC%' BEGIN
			IF @ncertificado IS NOT NULL AND @cramo IN (28, 38) BEGIN
				IF EXISTS (
					SELECT 1
					FROM adpoliza p
